package ch.globaz.tmmas.personnesservice.infrastructure.elasticsearch;
...
public class ElasticSearchClient {
    ...
    private static final String PERSONNES_INDEX = "personnes";
    ...
    public ResponseEntity findByUserName(String userName) throws IOException {
        ...
        GetResponse getResponse = client.get(new GetRequest(PERSONNES_INDEX, PERSONNES_INDEX, userName));

        if(getResponse.isExists()){
            return new ResponseEntity<PersonneIndex>(objectMapper.convertValue(getResponse.getSource(),PersonneIndex.class),
                    HttpStatus.OK);
        }
         return ResponseEntity.notFound().build();
    }

    public List<PersonneIndex> recherche(String methode,String terme) throws IOException {
        ...
        switch (methode){
            case "fuzzy":
                ...
                return fuzzy(fuzzySearchRequest(terme, PERSONNES_INDEX));
            case "wildcards":
                log.debug("wildcard search");
                return wildCard(wildCardsSearchRequest(terme, PERSONNES_INDEX));
            case "composed":
                log.debug("composed search");
                return composed(composedSearchRequest(terme, PERSONNES_INDEX));
            default:
    ...
    private List<PersonneIndex> composed(MultiSearchRequest searchRequest) throws IOException {
        ...
        List<PersonneIndex> personnes = Stream.concat(firstStream,secondStream)
                .map(hit -> {
                    log.info("Sync multisearch composed flux iteration for serialzation:; {}",hit.getSourceAsString());
                    try {
                        return objectMapper.readValue(hit.getSourceAsString(), PersonneIndex.class);
                    } catch (IOException e) {
                        e.printStackTrace();
                        return null;
                    }
    ...
    private List<PersonneIndex> wildCard(SearchRequest searchRequest) throws IOException {
    ...
    public List<PersonneIndex> fuzzy(SearchRequest searchRequest) throws IOException {
     ...
     * @return une Mono contenant potentiellement la réponse
     */
    public BulkResponse bulkIndex(List<PersonneIndex> docs) throws IOException {
    ...
    private BulkResponse bulkIndexDocs(List<PersonneIndex> docs) throws IOException {
    ...
    private IndexResponse doIndex(PersonneIndex doc) throws IOException {
        final IndexRequest indexRequest = new IndexRequest(PERSONNES_INDEX, PERSONNES_INDEX, doc.getNss());
        final String json = objectMapper.writeValueAsString(doc);
        indexRequest.source(json, XContentType.JSON);
        return client.index(indexRequest);
    }

    private BulkResponse doBulkIndex(List<PersonneIndex> docs) throws
            IOException {
        ...
        docs.forEach(doc -> {
            ...
            try {
                ...
                IndexRequest indexRequest = new IndexRequest(PERSONNES_INDEX, PERSONNES_INDEX, doc.getNss());
